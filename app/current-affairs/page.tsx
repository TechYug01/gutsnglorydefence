...
export const metadata = constructMetadata({ title: "Daily Current Affairs | Guts N Glory Defence" });
import { getDailyCurrentAffairs } from "@/lib/gdrive";
...
export default async function CurrentAffairsPage() {
  const files = await getDailyCurrentAffairs();
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h1 className="font-heading font-extrabold text-[clamp(2rem,5vw,3.5rem)] text-foreground mb-4 uppercase">
            Current <span className="text-gold">Affairs</span>
          </h1>
          <p className="text-muted text-lg max-w-2xl mx-auto">
            Stay updated with the latest happenings. Read our daily curated current affairs PDFs for your defence exam preparation.
          </p>
        </div>
        ...
        {files.length === 0 ? (
          <div className="bg-surface border border-edge rounded-2xl p-12 text-center shadow-lg">
            ...
            <h3 className="text-xl font-heading font-bold text-foreground mb-2">No PDF files found yet</h3>
            <p className="text-muted">
              We haven't uploaded any current affairs PDFs for today. Please check back later.
            </p>
          </div>
        ) : (
